...
useEffect(() => {
  const fetchProducts = async () => {
    try {
      ...
      const salesRes = await axiosInstance.get("/products?sortBy=sales_count&sortDirection=desc&limit=4");
      setBestSales(salesRes.data); // <-- update state

      // Fetch top 4 latest products by created_at
      const latestRes = await axiosInstance.get("/products?sortBy=created_at&sortDirection=desc&limit=4");
      setLatestProducts(latestRes.data); // <-- update state
    } catch (error) {
          ...
          {bestSales.length > 0 ? (
            bestSales.map((product) => (
              <div key={product.id} className="goods-card">
                <img src={product.image} alt={product.name} className="goods-image" />
                <h3>{product.name}</h3>
